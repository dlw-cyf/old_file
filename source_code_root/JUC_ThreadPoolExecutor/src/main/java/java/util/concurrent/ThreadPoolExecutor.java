...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    /**
     * 拒绝测试：JUC包提供了4中方式，默认采用 Abort..抛出异常的方式
     */
    ...
    private void advanceRunState(int targetState) {
        for (;;) {
            int c = ctl.get();
            /**
             * 条件一：runStateAtLeast(c, targetState)
             *      true：说明当前线程池状态 >= SHUTDOWN
             *      false：说明当前线程是RUNNING状态
             * 条件二：ctl.compareAndSet(c, ctlOf(targetState, workerCountOf(c)))
             *      条件成立：强制设置线程状态为 SHUTDOWN 状态
             */
    ...
    final void tryTerminate() {
        for (;;) {
            ...
            int c = ctl.get();
            /**
             * 条件一：isRunning(c)  判断线程是否是RUNNING状态
             *      说明线程池很正常。当前线程直接返回
             * 条件二：runStateAtLeast(c, TIDYING)
             *      说明有其他线程正在执行 TIDYING -> TERMINATED状态了，当前线程直接返回
             * 条件三：runStateOf(c) == SHUTDOWN && ! workQueue.isEmpty()
             *      说明队列中还有任务未执行完毕，线程池状态不能变化，直接返回
             */
            if (isRunning(c) ||
             ...
             * 什么情况下会执行到这里
             * 1.线程池状态 >= STOP
             * 2.线程池状态 == SHUTDOWN 且 队列已经为空
             */
            if (workerCountOf(c) != 0) { // Eligible to terminate
                /**中断一个空闲线程
                 * 空闲线程，在哪空闲呢？queue.tabke() || queue.poll()
                 * 1. 唤醒后的线程 会在getTask()方法返回null
                 * 2. 执行退出逻辑的时候会再次调用tryTerminate(),唤醒下一个空闲线程
                 * 3. 因为线程池状态是(线程池状态 >= STOP | 2.线程池状态 == SHUTDOWN 且 队列已经为空) 最终调用addWorker时，会失败
                 * 最终空闲线程都会在这里退出，非空闲线程执行完当前task时，也会调用tryTerminate方法，也有可能走到这里
                 * */
            ...
            /**
             * 执行到这里的线程是谁？
             * workerCountOf(c) == 0，会来到这里
             * 最后一个退出的线程。
             *      线程唤醒后，都会执行退出逻辑，退出过程中 会将workerCount计数-1 ==> ctl -1
    ...
    private void interruptIdleWorkers(boolean onlyOne) {
        ...
        try {
            for (Worker w : workers) {
                 ...
                 *      true：说明当前线程没有被中断
                 * 条件二：w.tryLock()
                 *      true：说明当前worker处于空闲状态(当前线程再queue.take() | queue.poll()阻塞中)，可以去给他一个中断信号
                 */
                if (!t.isInterrupted() && w.tryLock()) {
                    try {
                        /**给当前线程中断信号。处于queue阻塞的线程，会被唤醒
                         * 唤醒后，进入下一次自旋，可能会returnnull。执行退出相关逻辑
                         * */
                        t.interrupt();
                    } catch (SecurityException ignore) {
                    } finally {
                ...
                /**onlyOne == true：说明只中断一个线程，false：中断所有线程*/
                if (onlyOne)
                    break;
            }
        } finally {
     ...
     */
    //firstTask 可以为null，表示启动worker之后，worker自动到队列中获取任务，如果不是null，则worker优先执行firstTask
    //core 采用的线程数限制。true->采用核心线程数限制 false->采用maximumPoolSize线程数限制
    ...
    public ThreadPoolExecutor(int corePoolSize,//核心线程数量限制
                              ...
                              TimeUnit unit,//时间单位 second nano....
