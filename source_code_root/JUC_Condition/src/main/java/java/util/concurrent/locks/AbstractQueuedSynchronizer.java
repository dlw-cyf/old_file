...
public abstract class AbstractQueuedSynchronizer
    ...
    final boolean transferForSignal(Node node) {
         ...
         *          外部线程给前驱线程中断信号之后，前端node会将状态改为取消状态，并执行出队逻辑
         *      false：表示前驱节点修改成功。
         *
    ...
    final boolean transferAfterCancelledWait(Node node) {
        /**条件成立：说明当前Node一定是在条件队列内。Signal迁移节点到阻塞队列时，会将节点的状态修改为0*/
        if (compareAndSetWaitStatus(node, Node.CONDITION, 0)) {
            /**就算被中断也会进入到阻塞队列中*/
            enq(node);
            //返回true：表示在条件队列中断的
        ...
        /**
         * 什么时候会执行到这？
         * 1.当前node已经被外部线程调用Signal方法迁移到阻塞队列中。
         *      直接跳出循环，return false
         * 2.当前Node正在被外部线程调用Signal方法迁移到阻塞队列中。
         *      Thread.yield()：让出CPU，等待迁移到阻塞队列成功
         */
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
        ...
        private void unlinkCancelledWaiters() {
            ...
            while (t != null) {
                ...
                /**
                 * 条件成立：说明当前节点状态为不是条件队列的状态-2
                 *  1. 在node释放锁失败时，会将node状态设置为取消状态 1
                 *  2. node被中断信号唤醒的方式加入到阻塞队列中
                 *      会将状态设置为 0 但是不会清除node.nextWaiter关系。
                 * */
        ...
        private int checkInterruptWhileWaiting(Node node) {
            /**
             * Thread.interrupted()：返回当前线程中中断标记位，并重置当前标记位为false
             * */
            return Thread.interrupted() ?
                    /**transferAfterCancelledWait()：只有当前线程是被中断唤醒状态才回调用。*/
