...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
    ...
    public ConcurrentHashMap(int initialCapacity) {
        ...
        /**
         * 如果大于默认的最大容量的一半，就会使用最大容量(MAXIMUM_CAPACITY)
         * 否则通过tableSizeFor()获得初识容量
         */
    ...
    public ConcurrentHashMap(int initialCapacity,
        ...
        /**负载因子(loadFactor)和并发级别(concurrencyLevel) 都只是参与计算，并不会设置到静态属性中*/
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        for (Node<K,V>[] tab = table;;) {
            ...
            /**CASE3：前置条件：桶位的头结点一点不是null条件成立说明当前桶位的头结点为 FWD节点：说明当前Map正在扩容过程中*/
    ...
    private final Node<K,V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
             ...
             */
            if ((sc = sizeCtl) < 0)
                //大概率为-1，其他线程正在进行初始化操作。
                Thread.yield(); // lost initialization race; just spin
            else if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) {
                try {
                    /**
                     * 为什么这里要再次判断呢？
                     *  再当前线程进入while循环后，其他线程完成了初始化，
                     *  这时候当前线程执行 (sc = sizeCtl) < 0 为false，就会进入当前else if
                     *  如果不进行table的判断，就会再次初始化，丢失数据。
                     */
                    if ((tab = table) == null || tab.length == 0) {
                        /**
                         * table==null说明其他线程都没有对table初始化过。
                         *  sizeCtl = 0 ：表示创建table数组时，使用DEFAULT_CAPACITY为大小。
                         *  sizeCtl > 0
                         *      情况一：如果table未初始化，表示初始化大小
                         *      情况二：如果table已经初始化，表示下次扩容时 触发条件（阈值）
                         *  table==null说明未初始化，所以只有可能为情况一
                         */
                        int n = (sc > 0) ? sc : DEFAULT_CAPACITY;
                        @SuppressWarnings("unchecked")
                        Node<K,V>[] nt = (Node<K,V>[])new Node<?,?>[n];
                        //最终把新创建的table对象赋值给map.table
                        table = tab = nt;

                        //相当于 sc = 3/4 n : sc就是下次进行扩容的条件。
                        sc = n - (n >>> 2);
                    }
                } finally {
                    /**
                     * 1. 如果当前线程是第一创建map.table。sc表示的是下一次扩容的阈值。
                     * 2. 表示当前线程进来发现其他线程已经初始化过map.table了，
                     *    但是在else if条件中将sizeCtl设置为了-1，那么需要将sizeCtl设置为进入时的值。
                     */
    ...
    final Node<K,V>[] helpTransfer(Node<K,V>[] tab, Node<K,V> f) {
        /**
         * nextTable：引用的是 fwd.nextTable == map.nextTable理论上是这样的
         */
        Node<K,V>[] nextTab; int sc;

        /**
         * 条件一：tab != null 恒成立
         * 条件二：f instanceof 恒成立
         * 条件三：nextTab = ((ForwardingNode<K,V>)f).nextTable 恒成立。
         */
        if (tab != null && (f instanceof ForwardingNode) &&
            (nextTab = ((ForwardingNode<K,V>)f).nextTable) != null) {

            //通过当前数组的长度 获取当前扩容批次的唯一标识
            int rs = resizeStamp(tab.length);

            /**
             * 条件一：nextTab == nextTable
             *      true：标识当前扩容正在进行中
             *      false：1.nextTable在扩容完毕后设置为null。 2.再次触发扩容了，咱们拿到的nextTab也已经过期了。
             * 条件二：table == tab
             *      true：说明扩容正在进行中，还未完成。
             *      false：说明扩容已经完成了，扩容结束后，最后退出的线程会设置 nextTable 为 table。
             * 条件三：(sc = sizeCtl) < 0
             *      true：说明扩容正在进行
             *      false：说明sizeCtl是一个大于0的数，此时代码下次扩容的阈值，当前扩容已经结束。
             */
    ...
    private final void treeifyBin(Node<K,V>[] tab, int index) {
        Node<K,V> b; int n, sc;
        if (tab != null) {
            /**条件成立：说明当前table长度未达到64，此时不进行树化操作，进行扩容*/
                        ...
                        for (Node<K,V> e = b; e != null; e = e.next) {
                            TreeNode<K,V> p =
                                new TreeNode<K,V>(e.hash, e.key, e.val,
                                                  null, null);
                            /**
                             * 通过p.prev 和 tl.next将单向链表转换为TreeNode类型的双向节点。
                             */
                            if ((p.prev = tl) == null)
                                hd = p;
                            else
